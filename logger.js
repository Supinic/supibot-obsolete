(async function () {
	...
	Pool.getConnection().then(async (connector) => {
		...
		for (const obj of tables) {
			console.log("Setting ", obj.Tables_in_chat_line.replace(/#/, ""));
    ...
    client.on("message", async (evt) => {		
		...
		if (!CHANNEL_TABLES_BUFFER.has(channel)) {
			console.log("Adding a new channel", channel);
			for (const one of CHANNEL_TABLES_BUFFER.keys()) {
				console.log(one);
			}
	...
	setInterval(POST_DATA, 60e3); // once every 10s
})();