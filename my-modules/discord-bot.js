	...
	return class DiscordBot {
		constructor (ParentClient, Utils, key, channels, commands) {
			...
			this.discordClient.on("message", async (msg) => {
				...
				this.ircClient.checkAFK(user.toLowerCase(), null, replyObject);
				...
				if (cmd) {
					...
					else if (cmd.whitelist && !cmd.whitelist.some(i => i === discordChannel)) {
					...
					console.log("Discord command request: ", user, args);

					try {
						cmd.exec(user, args.slice(0), evt);
					}
					catch (e) {
						console.log("Discord command fail", e);
					...
					try {
						...
						console.log("Discord to twitch failed", e);
		...
		protectedIRCSend (channel, msg) {
			...
			this.ircClient.send(channel, this.utils.safeWrap(msg, 450).replace(/<:(.*?):(\d*)>/g, (total, emote) => emote));
		}
